...
export const HomePage: React.FC = () => {
  ...
  return (
    <div className="space-y-24 pb-24">
      ...
      <section className="relative h-[85vh] min-h-[620px] bg-charcoal text-white overflow-hidden">
          ...
          >
            ...
            <div className="relative z-10 h-full flex items-center justify-center text-center px-6">
              <div className="max-w-3xl space-y-6">
                <span className="text-xs uppercase tracking-[0.3em] text-brass font-bold inline-block">
                  Generational Craftsmanship
                </span>
                <h1 className="text-4xl md:text-6xl lg:text-7xl font-serif font-bold tracking-wide leading-tight text-parchment drop-shadow-md">
                  {slide.title}
                </h1>
                <p className="text-sm md:text-base text-gray-200 max-w-xl mx-auto font-light leading-relaxed">
                  {slide.subtitle}
                </p>
                <div className="pt-4">
                  <Link
                    to={slide.targetUrl || '/shop'}
                    className="inline-block bg-brass text-charcoal hover:bg-white px-10 py-4 text-xs font-bold uppercase tracking-[0.2em] transition duration-300 shadow-2xl"
                  >
                    Explore Collection
                  </Link>
                </div>
        ...
        {activeBanners.length > 1 && (
          <>
            <button
              ...
              className="absolute left-6 top-1/2 -translate-y-1/2 z-20 w-12 h-12 rounded-full bg-black/30 hover:bg-black/60 text-white backdrop-blur-sm flex items-center justify-center text-xl transition"
            >
              ‹
            </button>
            <button
              ...
              className="absolute right-6 top-1/2 -translate-y-1/2 z-20 w-12 h-12 rounded-full bg-black/30 hover:bg-black/60 text-white backdrop-blur-sm flex items-center justify-center text-xl transition"
            >
              ›
            </button>
              ...
              {activeBanners.map((_, i) => (
                <button
                  ...
                  className={`w-2.5 h-2.5 rounded-full transition-all duration-300 ${
                    i === currentSlide ? 'bg-brass w-8' : 'bg-white/50 hover:bg-white'
                  }`}
                />
              ))}
            </div>
      ...
      <section className="max-w-7xl mx-auto px-6">
        <div className="text-center max-w-2xl mx-auto mb-16 space-y-2">
          <span className="text-xs uppercase tracking-widest text-leather font-bold">Handpicked Silhouettes</span>
          <h2 className="text-3xl md:text-4xl font-serif font-bold text-charcoal">Featured Creations</h2>
          <p className="text-sm text-gray-500">Each piece is cut from the densest full-grain hides and stitched with waxed thread.</p>
        </div>
        ...
        {featured.length > 0 && (
          <div className="text-center pt-10">
            <Link
              to="/shop"
              className="inline-block border-2 border-charcoal text-charcoal px-8 py-3.5 text-xs font-bold uppercase tracking-widest hover:bg-charcoal hover:text-white transition"
            >
      ...
      <section className="bg-charcoal text-parchment py-24 px-6">
      ...
      <section className="max-w-7xl mx-auto px-6">
        <div className="text-center max-w-2xl mx-auto mb-16 space-y-2">
          <span className="text-xs uppercase tracking-widest text-leather font-bold">Curated Silhouettes</span>
          <h2 className="text-3xl font-serif font-bold text-charcoal">Shop by Collection</h2>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {categories.slice(0, 3).map((cat) => (
            <div key={cat.id} className="group relative h-96 overflow-hidden shadow-sm">
      ...
      {/* Newsletter Atelier */}
      <section className="max-w-4xl mx-auto px-6 text-center space-y-6 bg-parchment p-12 border border-leather/10">
        <span className="text-xs uppercase tracking-widest text-leather font-bold">Privé Circle</span>
        <h2 className="text-3xl font-serif font-bold text-charcoal">Subscribe to The Artisan Gazette</h2>
      ...
      <section className="max-w-7xl mx-auto px-6">
        <div className="bg-white border border-gray-200 overflow-hidden shadow-sm grid grid-cols-1 lg:grid-cols-3">
          {/* Atelier Details */}
          <div className="p-10 space-y-6 flex flex-col justify-center bg-parchment-light">
            <span className="text-xs uppercase tracking-widest text-leather font-bold">Visit Our Flagship</span>
            <h3 className="font-serif text-3xl font-bold text-charcoal">The Atelier & Showroom</h3>
          ...
          <div className="lg:col-span-2 h-96 lg:h-auto min-h-[380px] bg-gray-200">
            <iframe
